...
class ModelArtifact(_BaseModel):
    format: str = "GGUF"
    ...
    @classmethod
    def estimate(cls, total_params: int, quant: str = "Q4_K_M", fmt: str = "GGUF", source: _Optional[str] = None) -> "ModelArtifact":
        from modelscout.models.quantization import get_bits_per_weight
        bits = get_bits_per_weight(quant)
        bytes_val = int((total_params * bits / 8.0) + (200 * 1024 * 1024))
